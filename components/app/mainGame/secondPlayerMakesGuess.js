  ...
  useEffect(() => {
    if (validatedGuess.value) {
      const delay = setTimeout(
        () => dispatch({ type: "guessValidateAfterDelay" }),
        750
      );
        ...
        {makeGuess ? (
          <div id={styles.formArea}>
            ...
            <p id={styles.errorMessage}>
              {" "}
              {validatedGuess.foundError
                ? validatedGuess.errorMessage
                : ""}{" "}
            </p>
            <DarkButton id={styles.guessBtn} onClick={SendGuess}>
              Show {firstPlayer}
            </DarkButton>
          </div>
        ) : (
          <div>
            <p className={`${styles.textArea} hint`}>
              Note: You will lose a guess whether you choose to make a guess or
              not
            </p>
            <RedButton className={styles.button} onClick={DontSendGuess}>
              {" "}
              Skip Guess{" "}
            </RedButton>
            <GreenButton
              className={styles.button}
              onClick={() => setMakeGuess(true)}
            >
              {" "}
              Make Guess{" "}
            </GreenButton>
